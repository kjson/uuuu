...
# Example: Process log stream using your library
def process_logs():
    # Generate a stream of logs
    logs = generate_logs()

    # Step 1: Split the logs into errors and non-errors
    error_logs, non_error_logs = iterables.split(lambda log: log['level'] == 'ERROR', logs)
    ...
    rolling_error_counts = iterables.rolling_aggregate(error_batches, lambda acc, batch: acc + len(batch), initial=0)
